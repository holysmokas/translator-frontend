...
(function () {
    ...
    async function joinRoomAsGuest(roomCode, name, language) {
        ...
        try {
            ...
            state.maxMinutes = data.max_minutes || 5; // Use host's remaining time
            ...
            console.log(`🎥 Guest joining with video mode: ${state.videoMode} (host tier: ${state.hostTier}), maxMinutes: ${state.maxMinutes}`);
    ...
    function updatePlanDisplay(data) {
        ...
        if (data.usage && data.limits) {
            ...
            if (roomsLimit === -1) {
                ...
                const remaining = Math.max(0, roomsLimit - roomsUsed);
                if (remaining === 0) {
                    elements.planUsage.textContent = 'Trial used';
                    elements.planUsage.style.color = '#ef4444'; // Red
                } else {
                    elements.planUsage.textContent = `${remaining} room${remaining !== 1 ? 's' : ''} remaining`;
                    elements.planUsage.style.color = ''; // Reset to default
                }
            }
        }
    ...
    window.toggleMute = function () {
        ...
        if (state.useP2P && typeof WebRTCP2P !== 'undefined') {
            // P2P WebRTC mode
            const isEnabled = WebRTCP2P.toggleAudio();
            ...
            console.log('🎤 Daily.co Mic toggled:', !isCurrentlyMuted ? 'ON' : 'OFF');

            if (micBtn) {
                micBtn.classList.toggle('muted', isCurrentlyMuted);
                micBtn.innerHTML = !isCurrentlyMuted
                    ? '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>'
                    : '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 11h-1.7c0 .74-.16 1.43-.43 2.05l1.23 1.23c.56-.98.9-2.09.9-3.28zm-4.02.17c0-.06.02-.11.02-.17V5c0-1.66-1.34-3-3-3S9 3.34 9 5v.18l5.98 5.99zM4.27 3L3 4.27l6.01 6.01V11c0 1.66 1.33 3 2.99 3 .22 0 .44-.03.65-.08l1.66 1.66c-.71.33-1.5.52-2.31.52-2.76 0-5.3-2.1-5.3-5.1H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c.91-.13 1.77-.45 2.54-.9L19.73 21 21 19.73 4.27 3z"/></svg>';
            }
        }
    ...
    window.toggleVideo = function () {
        ...
        if (state.useP2P && typeof WebRTCP2P !== 'undefined') {
            // P2P WebRTC mode
            const isEnabled = WebRTCP2P.toggleVideo();
            ...
            console.log('📹 Daily.co Camera toggled:', !isCurrentlyOff ? 'ON' : 'OFF');

            if (videoBtn) {
                videoBtn.classList.toggle('muted', isCurrentlyOff);
                videoBtn.innerHTML = !isCurrentlyOff
                    ? '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>'
                    : '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M21 6.5l-4 4V7c0-.55-.45-1-1-1H9.82L21 17.18V6.5zM3.27 2L2 3.27 4.73 6H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.21 0 .39-.08.54-.18L19.73 21 21 19.73 3.27 2z"/></svg>';
            }
        }
    ...
    window.rejoinFromHistory = async function (roomCode, sessionId) {
        rejoinActiveSession({
            ...
            // max_minutes will come from API response
        });
    };
    ...
    async function createInvite() {
        ...
        try {
            ...
            if (response.status === 402 || response.status === 429) {
                ...
                // Refresh profile to update plan display
                await loadProfile();
                return;
            }
            ...
            state.pendingMaxMinutes = data.max_minutes || 5;
    ...
    async function rejoinActiveSession(sessionData) {
        ...
        try {
            ...
            // Get video URL and correct max_minutes
            ...
            if (response.ok && data.video_url) {
                // Use max_minutes from response (tier-based), fallback to remaining_minutes, then 5 (trial default)
                state.maxMinutes = data.max_minutes || sessionData.remaining_minutes || 5;
                console.log('🔄 Rejoin max_minutes:', state.maxMinutes, 'tier:', data.tier);
                connectWebSocket(data.video_url);
            } else {
    ...
    function downloadCalendarFile(roomCode = null) {
        ...
        const maxMins = roomCode ? (state.pendingMaxMinutes || 5) : state.maxMinutes;
        const message = getInviteMessage();
...
================================`;
    ...
    function showRoomUI(videoUrl) {
        ...
        // Update file sharing UI now that we know if P2P is active
        // (Screen share only available with Daily.co, not P2P)
        updateFileSharingUI();
    ...
    // Screen share available for ALL tiers (including trial and starter)
    const SCREEN_SHARE_TIERS = ['trial', 'starter', 'professional', 'business', 'enterprise', 'payperminute'];
    ...
    // Check if user can share screen (requires Daily.co, not P2P)
    function canShareScreen() {
        // Screen share requires Daily.co video, not available on P2P
        if (state.useP2P) {
            return false;
        }
        const tier = state.profile?.tier || 'trial';
        return SCREEN_SHARE_TIERS.includes(tier);
    }
    ...
    function updateFileSharingUI() {
        ...
        const canScreen = canShareScreen();

        console.log('📁 updateFileSharingUI called:', { tier, canShare, canScreen, profile: state.profile });
        ...
        // Show/hide screen share button - available for ALL tiers!
        if (elements.toggleScreenShareBtn) {
            elements.toggleScreenShareBtn.style.display = canScreen ? '' : 'none';
            console.log('🖥️ screenShareBtn display:', canScreen ? 'visible' : 'hidden');
        }
    }
    ...
    async function toggleScreenShare() {
        if (!canShareScreen()) {
            showNotification('Screen sharing not available for your account', 'error');
            return;
        }
