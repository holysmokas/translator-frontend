        ...
        switch (data.type) {
            case 'system':
                ...
                if (data.existing_participants) {
                    ...
                    // ADD existing participants to participants map with their correct language
                    state.existingParticipants.forEach(peer => {
                        if (!state.participants.has(peer.user_id)) {
                            console.log(`👤 Adding existing participant: ${peer.user_name} (${peer.language})`);
                            state.participants.set(peer.user_id, {
                                name: peer.user_name,
                                language: peer.language || 'en',
                                isMuted: false,
                                isSpeaking: false,
                                isHost: true  // They were here first, so they're the host
                            });
                        }
                    });
                    updateParticipantsUI();
