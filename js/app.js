...
================================`;
    ...
    function showRoomUI(videoUrl) {
        ...
        // Use P2P for starter/trial tiers (max 2 participants)
        const tier = state.profile?.tier || 'trial';
        const useP2P = (tier === 'trial' || tier === 'starter') && typeof WebRTCP2P !== 'undefined';
        state.useP2P = useP2P;

        console.log(`🎥 Video mode: ${useP2P ? 'P2P WebRTC' : 'Daily.co'} (tier: ${tier})`);
    ...
    async function initP2PVideo(videoGrid) {
        ...
        if (initialized) {
            ...
            // If there are existing participants, initiate call
            if (state.existingParticipants && state.existingParticipants.length > 0) {
                const peer = state.existingParticipants[0];
                console.log('📞 Calling existing peer:', peer.user_name);
                WebRTCP2P.startCall(peer.user_id, peer.user_name);
            }
        } else {
